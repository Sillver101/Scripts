...
run_step "Writing Fail2Ban configuration" bash -c "cat > /etc/fail2ban/jail.local << 'EOF'
[sshd]
enabled = true
port = 22
backend = systemd
...
run_step "Updating ClamAV signatures" bash -c "
systemctl stop clamav-freshclam || true
freshclam || true
systemctl restart clamav-daemon || true
"

run_step "Creating website root" bash -c "
...
run_step "Creating SSL helper script" bash -c "
...
run_step "Restarting services" bash -c "
